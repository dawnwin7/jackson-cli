...
export class DrizzleRepository implements Repository {
  ...
  async findUserByTokenHash(tokenHash: string) {
    const [user] = await this.db.select().from(users).where(eq(users.tokenHash, tokenHash)).limit(1)
    return user ?? null
  }

  async createRequest(userId: string, message: string) {
    const id = `req_${randomUUID().replaceAll('-', '').slice(0, 16)}`
    const [request] = await this.db.insert(requests).values({ id, userId, message }).returning()
  ...
  async storeTelegramMessage(requestId: string, operatorChatId: number, sentMessageId: number) {
    await this.db
  ...
  async findRequestByTelegramMessage(operatorChatId: number, sentMessageId: number) {
