  ...
  async sendMessage(chatId: number, text: string): Promise<number> {
    ...
    try {
      ...
      try {
        ...
        throw new TelegramSendError('telegram returned non-json response', { cause: error })
      }
      ...
      if (response.status >= 400 || body?.ok !== true) {
        ...
        throw new TelegramSendError(
          typeof description === 'string' ? description : 'telegram sendMessage failed',
        )
      }
      ...
      if (!Number.isSafeInteger(messageId)) {
        throw new TelegramSendError('telegram sendMessage response missing message_id')
      }
