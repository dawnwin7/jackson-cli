...
const replyCommandPattern =
  /^\/reply\s+(?<requestId>\S+)(?:\s+(?<reply>[\s\S]*))?$/;
...
export function createApp({
  ...
  app.post("/cli/login", async (context) => {
    ...
    try {
      ...
      return context.json(
        {
          detail:
            error instanceof Error ? error.message : "username is required",
        },
        400,
      );
    }
  ...
  app.get("/cli/requests/:request_id", async (context) => {
    ...
    const timeoutSeconds = parseFloatQuery(
      context.req.query("timeout_seconds"),
      15,
    );
    ...
    while (true) {
      const request = await repository.getRequestForUser(
        requestId,
        authenticated.user.id,
      );
      if (!request) {
