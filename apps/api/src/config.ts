...
export function loadRuntimeConfig(
  ...
  if (testMode) {
    return {
      telegramBotToken:
        nonEmpty(env.JACKSON_TELEGRAM_BOT_TOKEN) ?? TEST_BOT_TOKEN,
      telegramOperatorChatId:
        parseOptionalChatId(env.JACKSON_TELEGRAM_OPERATOR_CHAT_ID) ??
        TEST_OPERATOR_CHAT_ID,
      telegramWebhookSecret: nonEmpty(env.JACKSON_TELEGRAM_WEBHOOK_SECRET),
      telegramApiBaseUrl:
        nonEmpty(env.JACKSON_TELEGRAM_API_BASE_URL) ??
        DEFAULT_TELEGRAM_API_BASE_URL,
      telegramUpdateMode: parseTelegramUpdateMode(
        env.JACKSON_TELEGRAM_UPDATE_MODE,
      ),
      testMode: true,
  ...
  return {
    ...
    telegramOperatorChatId: parseRequiredChatId(
      env.JACKSON_TELEGRAM_OPERATOR_CHAT_ID,
    ),
    telegramWebhookSecret: nonEmpty(env.JACKSON_TELEGRAM_WEBHOOK_SECRET),
    telegramApiBaseUrl:
      nonEmpty(env.JACKSON_TELEGRAM_API_BASE_URL) ??
...
function parseTelegramUpdateMode(
  value: string | undefined,
): TelegramUpdateMode {
  const mode = nonEmpty(value) ?? DEFAULT_TELEGRAM_UPDATE_MODE;
  if (mode !== "polling" && mode !== "webhook") {
    throw new Error(
      'JACKSON_TELEGRAM_UPDATE_MODE must be "polling" or "webhook"',
    );
  }
  return mode;
}
