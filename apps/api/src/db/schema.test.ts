...
describe('Drizzle schema metadata', () => {
  ...
  it('stores only token hashes and defines the required uniqueness constraints', () => {
    ...
    expect(userConfig.indexes.map(({ config }) => [config.name, config.unique])).toEqual(
    ...
    expect(replyConfig.primaryKeys.map(({ name }) => name)).toContain('uniq_reply_per_request')
    expect(replyConfig.indexes.map(({ config }) => [config.name, config.unique])).toContainEqual([
      'uniq_reply_update_id',
      true,
    ])
    ...
    expect(messageConfig.indexes.map(({ config }) => [config.name, config.unique])).toContainEqual([
      'uniq_operator_sent_message',
      true,
    ])

    expect(getTableConfig(telegramWebhookUpdates).primaryKeys.map(({ name }) => name)).toContain(
      'uniq_telegram_update_id',
    )
  })
})
